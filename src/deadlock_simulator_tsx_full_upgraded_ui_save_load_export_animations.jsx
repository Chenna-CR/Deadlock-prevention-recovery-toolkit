...
interface Proc {
  ...
  x: number;
  y: number;
  allocated: string[]; // resource ids (can contain duplicates => multiple instances)
  requesting: string[]; // resource ids (can contain duplicates)
  maxClaim: Record<string, number>; // per-resource maximum claim for Banker's
}
...
// Utility to count occurrences of resource ids in an array
const countMap = (arr: string[]) => {
  const m: Record<string, number> = {};
  for (const r of arr) m[r] = (m[r] || 0) + 1;
  return m;
};
...
const DeadlockSimulator: React.FC = () => {
  ...
  const addProcess = (x: number, y: number) => {
    ...
    setProcesses((s) => [
      ...s,
      { id, x, y, allocated: [], requesting: [], maxClaim: {} },
    ]);
  };
    ...
    if (mode === "connect") {
      if (!connectingFrom) {
        ...
        if (newEdge.type === "request") {
          ...
          // resource -> process : an allocation (one instance)
          setProcesses((arr) =>
            arr.map((p) =>
              p.id === newEdge.to
  ...
  // ----------------------------
  // bankersCheck returns { safe, sequence }
  const bankersCheck = () => {
    // Build available map
    const avail: Record<string, number> = {};
    resources.forEach((r) => (avail[r.id] = r.available));

    // Build per-process alloc and request counts
    const allocs: Record<
      string,
    ...
    for (const p of processes) {
      allocs[p.id] = countMap(p.allocated);
      reqs[p.id] = countMap(p.requesting);
    }

    // Determine per-process maxClaim for algorithm (if not present, infer as alloc + req)
    const maxClaims: Record<string, Record<string, number>> = {};
    for (const p of processes) {
      const m: Record<string, number> = {};
      // if user specified maxClaim entries use them
      if (p.maxClaim && Object.keys(p.maxClaim).length > 0) {
        for (const rid of Object.keys(p.maxClaim)) {
          m[rid] = p.maxClaim[rid];
        }
      } else {
        // infer: for any resource observed in alloc or req, set max = alloc + req
        const keys = new Set<string>([
          ...Object.keys(allocs[p.id] || {}),
          ...Object.keys(reqs[p.id] || {}),
        ]);
        for (const k of keys) {
          m[k] = (allocs[p.id]?.[k] || 0) + (reqs[p.id]?.[k] || 0);
        }
      }
      maxClaims[p.id] = m;
    }

    // Build need = maxClaim - allocated
    const need: Record<string, Record<string, number>> = {};
    for (const p of processes) {
      need[p.id] = {};
      const mc = maxClaims[p.id] || {};
      const al = allocs[p.id] || {};
      for (const rid of Object.keys(mc)) {
        const n = (mc[rid] || 0) - (al[rid] || 0);
        need[p.id][rid] = Math.max(0, n);
      }
    }

    // Banker's safety algo
    const work: Record<string, number> = { ...avail };
    ...
    while (progressed) {
      ...
      for (const p of processes) {
        ...
        const pNeed = need[p.id];
        // Check if need <= work for all resources in need
        const ok = Object.keys(pNeed).every((rid) => (pNeed[rid] || 0) <= (work[rid] || 0));
        if (ok) {
          // simulate releasing allocated resources
          const a = allocs[p.id] || {};
          for (const rid of Object.keys(a)) {
            work[rid] = (work[rid] || 0) + (a[rid] || 0);
          }
          finish[p.id] = true;
          seq.push(p.id);
          progressed = true;
    ...
    const allFinished = processes.every((p) => finish[p.id]);
    return { safe: allFinished, sequence: allFinished ? seq : [] as string[] };
  };

  const calculateSafeSequence = () => {
    const res = bankersCheck();
    return res.sequence;
  };

  // WFG cycle detection (processes only)
  const detectCycleWFG = () => {
  ...
  const detectDeadlock = () => {
    ...
    const wfg = detectCycleWFG();

    // run Banker's safety check
    const banker = bankersCheck();

    setDeadlockDetected(wfg.found);
    setCycleNodes(wfg.cycle || []);
    setSafeSequence(banker.sequence);

    if (wfg.found) {
      setSystemState("unsafe");
      setStatusMessage(
        `DEADLOCK DETECTED (cycle in WFG): ${wfg.cycle.join(" → ")}`
      );
    } else if (!banker.safe) {
      setSystemState("unsafe");
      setStatusMessage(`System is UNSAFE (Banker's check failed)`);
    } else {
      setSystemState("safe");
      setStatusMessage(`No deadlock • System is SAFE (Banker's OK)`);
    }
  };
  ...
  const onLoadFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // normalize if older format: ensure maxClaim exists on processes
      const ps: Proc[] = (data.processes || []).map((p: any) => ({
        id: p.id,
        x: p.x,
        y: p.y,
...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
  ...
  // helper to update a process's maxClaim when user edits
  const updateProcessMaxClaim = (pid: string, rid: string, value: number) => {
    setProcesses((ps) =>
      ps.map((p) =>
        p.id === pid ? { ...p, maxClaim: { ...p.maxClaim, [rid]: Math.max(0, value) } } : p
      )
    );
  };
              ...
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <div className="bg-white/90 dark:bg-slate-800/70 p-8 rounded-2xl shadow-2xl max-w-md border border-white/60 backdrop-blur">
                  ...
                  <ol className="space-y-2 text-gray-700 dark:text-gray-300">
                    ...
                    <li>2. Connect them together (Process→Resource = REQ, Resource→Process = ALLOC)</li>
                    <li>3. Set each process's max claim in the right panel (for Banker's)</li>
                    <li>4. Detect deadlocks and view prevention suggestions</li>
                  </ol>
                </div>
              </div>
            )}
              ...
              <div className="rounded-xl bg-white dark:bg-slate-800 p-3 shadow">
                ...
                {selectedNode.type === "process" ? (
                  <div className="space-y-2 text-xs text-gray-600 dark:text-gray-300">
                    ...
                    <div className="mt-2">
                      <div className="text-sm font-medium mb-1">Max Claims (per resource)</div>
                      <div className="space-y-2 max-h-40 overflow-y-auto">
                        {resources.length === 0 && <div className="text-xs text-gray-500">Add resources to set max claims</div>}
                        {resources.map((r) => {
                          const p = selectedNode.node as Proc;
                          const v = p.maxClaim?.[r.id] ?? 0;
                          return (
                            <div key={r.id} className="flex items-center gap-2">
                              <div className="w-12 text-xs">{r.id}</div>
                              <input
                                type="number"
                                min={0}
                                value={v}
                                onChange={(e) => updateProcessMaxClaim(p.id, r.id, parseInt(e.target.value || "0"))}
                                className="w-20 px-2 py-1 rounded border bg-white/80 dark:bg-slate-900/60 text-xs"
                              />
                              <div className="text-xs text-gray-500">allocated:{(p.allocated.filter(a => a===r.id).length)}</div>
                            </div>
                          );
                        })}
                      </div>
                      <div className="text-xs text-gray-500 mt-2">If you leave max claims empty, the system will infer max = allocated + requested (for demonstration).</div>
                    </div>
                  </div>
                ) : (
